...
<center>客户端案列分析</center>

1. Redis内存徒增。
> 1. 服务端：Redis主节点内存徒增，几乎满maxmemory。而从节点内存并没有变化。
> 2. 客户端：客户端产生了OOM异常，也就是Redis主节点使用的内存已经超过maxmemory，无法写入新的数据。
> 3. 分析原因
>> 1. 确实有大量的写入，但是主从复制出现问题。（分别查看主从的key个数 dbsize）
>> 2. 其他原因造成主节点内存使用过大，排查是否由客户端缓冲区造成主节点内存陡增，使用info clients 查询先关信息
> 4. 处理方法：使用client kill 杀掉这个连接即可。
> 5. 处理方法和后期处理。
>> 1. 运维层面：禁止monitor命令，或者重命名次命令。
>> 2. 使用专业的Redis运维工具，限制输出缓冲区的大小。

2. 客户端周期性的超时
> 1. 现象
>>1. 客户端：客户端出现大量超时，经过分析发现超时是周期性出现的。
>>2. 服务端：服务端并没有明显的异常，只是有一些慢查询操作。
>2. 分析
>> 1. 网络原因：服务端和客户端之间的网络出现周期性问题，经过观察网络是正常现象
>>2. Redis本身：经过观察Redis日志统计，没有发现异常
>> 3. 客户端：由于周期性出现问题，就和慢查询日志的历史记录对应了一下时间，发现只要慢查询出现，客户端就会产生大量连接超时。两个时间基本一致。

<center>本章回顾</center>

1. Jedis.close() 在直连下是关闭连接，在连接池下是归还连接。
2. Jedis 没有内置序列化，需要自己选用
3. 客户端输入缓存区不能配置，默认1G。
4. monitor命令虽然好用，但是在大并发下存在输出缓冲区暴涨的可能性。
5. info clients 帮助开发和运维人员找到客户端可能存在的问题。


<center>持久化</center>

RDB： rdb持久化是把当前进程数据生产快照保存到硬盘的过程，触发RDB持久化过程分为手动触发和自动触发

1. 手动触发： save 和 bgsave
> 1. save命令：阻塞当前Redis服务器，知道RDB过程完成为止。对于内存比较大的实例会造长时间的阻塞。线上环境不建议使用。
> 2. bgsave命令：Redis进程执行fork操作创建子进程，RDB持久化过程由子进程负责。阻塞执法撒恒在fork阶段，一般时间很短。
2. 自动触发
3. RDB文件的处理：RDB文件保存在dir配置指定的目录下，文件名通过dbfilename配置指定。可以通过执行config set newDir 运行期动态执行。当下次运行RDB时文件会保存到新目录。
4. 压缩：Redis默认采用LZF算法对生成的RDB文件做压缩处理，压缩后的文件远远小于内存大小。可以通过 config set rdbcompression {yes|no} 动态修改。
5. RDB的优缺点
> 1. 优点：
>> 1. 备份， 二进制文件，加载RDB恢复数据远远快于AOF的方式。
> 2. 缺点：
>> 1. 无法做到实时持久化、妙计持久化。每次fork新进程。重量级操作，频繁执行成本过高。
>> 2. 版本演进过程中，格式变化多。不兼容问题。

AOF: 以独立日志的方式记录每次写命令，重启时在重新执行AOF文件中的命令达到恢复数据的目的。AOF的作用主要是解决数据持久化的实时性。目前的主流方式。